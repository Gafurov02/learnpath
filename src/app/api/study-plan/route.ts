...
export async function GET(req: NextRequest) {
    ...
    if (!hasMaxAccess(sub)) {
        return NextResponse.json({ error: 'Max required' }, { status: 403 });
    }
...
- Weak topics: ${weakTopics.map((t) => `${t.exam}/${t.topic} (${t.accuracy}%)`).join(', ') || 'none yet'}
...
function defaultPlan() {
    return {
        ...
        tasks: [
            {
                ...
                description: 'Build your academic word bank',
                exam: 'IELTS',
            ...
            },
            {
                ...
                description: 'Practice extracting key information',
                exam: 'IELTS',
        ...
        daily_goal: 20,
        tip: 'Consistency beats intensity. 20 minutes daily beats 2 hours once a week.',
    };
}