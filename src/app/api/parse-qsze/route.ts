...
function tryJson(text: string) {
  ...
  if (questions.length === 0) throw new Error('JSON has no questions array');
...
function tryQst(text: string) {
  ...
  for (const line of lines) {
    ...
    if (marker === '?' && value) {
      if (current) {
        candidates.push({ question: current.question, options: current.options, correct_index: current.correctIndex });
      }
    ...
    if ((marker === '+' || marker === '-') && current && value) {
      if (marker === '+' && current.correctIndex === -1) current.correctIndex = current.options.length;
  ...
  if (candidates.length === 0) throw new Error('QST questions not found');

  return candidates.map(toParsedQuestion);
}

function readUInt32LE(data: Buffer, state: { pos: number }) {
  if (state.pos + 4 > data.length) throw new Error('Unexpected end of file');
...
function readString(data: Buffer, state: { pos: number }) {
  ...
  if (length > data.length - state.pos) throw new Error('String length is outside file bounds');
...
function tryAssystQsz(data: Buffer) {
  ...
  while (state.pos < data.length) {
    while (state.pos < data.length && data[state.pos] === 0) state.pos += 1;
    if (data.length - state.pos < 8) break;

    const question = readString(data, state);
    const answerCount = readUInt32LE(data, state);
    if (!question || answerCount < 2 || answerCount > 32) throw new Error('Invalid QSZ structure');
    ...
    for (let answerIndex = 0; answerIndex < answerCount; answerIndex += 1) {
      ...
      if (answer) {
        if (flag !== 0 && correctIndex === -1) correctIndex = options.length;
        options.push(answer);
      }
    }
  ...
  if (candidates.length === 0) throw new Error('QSZ questions not found');

  return candidates.map(toParsedQuestion);
}

function tryLegacyQsze(data: Buffer) {
  const attempts = [92, 0, 4, 16, 24, 32];
  ...
  for (const start of attempts) {
    try {
      ...
      for (let questionIndex = 0; questionIndex < 500; questionIndex += 1) {
        if (state.pos + 8 >= data.length) break;
        if (questionIndex > 0 && state.pos + 4 <= data.length && data.readUInt32LE(state.pos) === 0) {
          state.pos += 4;
        }

        const questionLength = data.readUInt32LE(state.pos);
        if (questionLength === 0 || questionLength > 5000 || state.pos + 4 + questionLength > data.length) break;
        const question = readString(data, state);
        state.pos += 8;
        const answerCount = readUInt32LE(data, state);
        const correctIndex = readUInt32LE(data, state);
        if (answerCount < 2 || answerCount > 32) break;

        const options: string[] = [];
        for (let answerIndex = 0; answerIndex < answerCount; answerIndex += 1) {
          const answerLength = data.readUInt32LE(state.pos);
          if (answerLength === 0 || answerLength > 5000 || state.pos + 4 + answerLength > data.length) break;
      ...
      if (candidates.length > 0) return candidates.map(toParsedQuestion);
    } catch {
      // Try the next known layout.
    }
  }

  throw new Error('Legacy QSZE questions not found');
}
...
function getBinaryCandidates(input: Buffer) {
  ...
  for (const [name, decompress] of [
    ...
    try {
      ...
      // Different exporters wrap QSZE differently; unsupported wrappers are ignored.
    }
  }
...
function parseQuestions(input: Buffer) {
  ...
  for (const candidate of getBinaryCandidates(input)) {
    const text = candidate.data.toString('utf8').replace(/^\uFEFF/, '').trim();

    if (text) {
      for (const [name, parse] of [['JSON', tryJson], ['QST', tryQst]] as const) {
        try {
    ...
    for (const [name, parse] of [['Assyst QSZ', tryAssystQsz], ['Legacy QSZE', tryLegacyQsze]] as const) {
      try {
  ...
  throw new Error(`Не удалось распознать файл. Поддерживаются QST, QSZ/QSZE и JSON. ${errors.slice(0, 4).join('; ')}`);
}
