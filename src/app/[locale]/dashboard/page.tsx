...
import { AppNavbar } from '@/components/layout/AppNavbar'; // Fixed: was landing <Navbar>
...
import { ALL_EXAMS } from '@/lib/limits'; // Fixed: was hardcoded local array
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      if (!session) {
        // Fixed: was redirecting to /home causing a circular redirect loop
      ...
      if (progressResponse.ok) {
        ...
        data.attempts.forEach((attempt) => {
          if (!byExam[attempt.exam]) byExam[attempt.exam] = { total: 0, correct: 0 };
          byExam[attempt.exam].total += 1;
          if (attempt.correct) byExam[attempt.exam].correct += 1;
        });
        setExamStats(byExam);
      }
  ...
  return (
      ...
      >
        {/* Fixed: was using landing <Navbar> for an authenticated page */}
        <AppNavbar />
        ...
        >
          ...
          <div style={{ marginBottom: 32 }}>
            <div style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))', marginBottom: 4 }}>
              {t('welcome')}
            </div>
            <h1
                style={{
          ...
          >
            {statCards.map((s) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 30, fontWeight: 500, color: s.color, marginBottom: 2 }}>
                    {s.value}
                  </div>
          ...
          {Object.keys(examStats).length > 0 && (
              ...
              >
                <h2 style={{ fontSize: 15, fontWeight: 500, marginBottom: 16, margin: '0 0 16px' }}>
                  Progress by exam
                </h2>
                          ...
                          <div style={{ height: 6, background: 'hsl(var(--border))', borderRadius: 3 }}>
                            <div
                                style={{
                                  height: 6,
                                  background:
                                      pct >= 70 ? '#22C07A' : pct >= 40 ? '#EF9F27' : '#E84040',
                                  borderRadius: 3,
                                  width: `${pct}%`,
              ...
              >
                <div>
                  <div style={{ fontSize: 15, fontWeight: 500, marginBottom: 2 }}>
                    ✓ {isMax ? 'Max' : 'Pro'} plan active
                  </div>
                  <div style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))' }}>
                    {isMax
                        ? 'Unlimited questions · AI Tutor · Day-by-day plan'
                        : '50 questions / 3 days · All exams · Full AI explanations'}
                  </div>
                </div>
              </div>
          ) : (
          ...
          {/* Choose exam — Fixed: was hardcoded, now uses ALL_EXAMS from limits.ts */}
          ...
          >
            {ALL_EXAMS.map((examName) => {
              const s = examStats[examName];
              const pct = s ? Math.round((s.correct / s.total) * 100) : null;
              return (
                  <Link
                      key={examName}
                      href={`/${locale}/quiz?exam=${examName}`}
                      style={{
                        background: 'hsl(var(--card))',
                        border: '1px solid hsl(var(--border))',
                        borderRadius: 12,
                  ...
                  >
                    <div style={{ fontSize: 15, fontWeight: 500, marginBottom: 4 }}>{examName}</div>
                    <div
                        style={{
                          fontSize: 11,
