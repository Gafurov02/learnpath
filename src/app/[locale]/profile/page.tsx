...
/** Resize avatar file to 192×192 JPEG before storing */
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
            ...
            image.onload = () => {
                ...
                const ox = (image.width - smallest) / 2;
                const oy = (image.height - smallest) / 2;
                ...
                ctx.drawImage(image, ox, oy, smallest, smallest, 0, 0, size, size);
...
export default function ProfilePage() {
    ...
    // ─── Responsive state ─────────────────────────────────────────────────────
    // FIXED: was using window.innerWidth directly in JSX (SSR crash + hydration mismatch).
    // Default to false so server and initial client render match, then update after mount.
    const [isMobile, setIsMobile] = useState(false);
    const [isNarrow, setIsNarrow] = useState(false); // < 1024px (sidebar collapses)
    ...
    useEffect(() => {
        ...
        supabase.auth.getSession().then(async ({ data: { session } }) => {
            ...
            const dates = [
                ...new Set(
                    (attemptsRes.data ?? []).map((a: { created_at: string }) =>
            ...
            const earnedMap = new Map(
                (achRes.data ?? []).map((a: { achievement: string; earned_at: string }) => [
                    a.achievement,
                    a.earned_at,
                ])
            );
            ...
            setAchievements(
                (allAchRes.data ?? []).map((a: { code: string; name: string; description: string; icon: string }) => ({
                    code: a.code,
                    name: a.name,
    ...
    async function handleLogout() {
        ...
        // Fixed: was window.location.href (full reload). router.push is cleaner.
        router.push(`/${locale}`);
        router.refresh();
    }
    ...
    async function handleSaveProfile() {
        ...
        try {
            ...
            // NOTE: avatarUrl here may be a base64 string (up to ~150 KB).
            // For production consider uploading to Supabase Storage and saving the URL instead.
    ...
    const dailyQuests = [
        { title: 'Solve 20 questions', progress: total, target: 20, reward: 120 },
        { title: 'Reach 80% accuracy', progress: accuracy, target: 80, reward: 200 },
        { title: 'Maintain streak', progress: streak, target: 7, reward: 250 },
    ];
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        last7.push({
            date: d.toLocaleDateString(locale === 'ru' ? 'ru-RU' : 'en-US', { weekday: 'short' }),
            count: attempts.filter((a) => new Date(a.created_at).toDateString() === dateStr).length,
        });
    }
        ...
        >
            ...
            <main
                style={{
                    ...
                    maxWidth: 1320,
                    margin: '0 auto',
                    // FIXED: was window.innerWidth < 768 directly — now uses isMobile state
                    padding: isMobile ? '20px 14px 120px' : '40px 24px 120px',
                    display: 'flex',
            ...
            >
                ...
                <ScaleIn delay={0.05}>
                    <div
                        ...
                        style={{
                            ...
                            padding: 24,
                            display: 'flex',
                            flexDirection: isMobile ? 'column' : 'row', // FIXED: was window.innerWidth
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                            ...
                            <div>
                                ...
                                <div style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))' }}>
                                    {locale === 'ru' ? 'Управление профилем' : 'Manage your profile'}
                                </div>
                            </div>
                        </div>
                        ...
                        >
                            <input
                                ...
                                style={{
                                    // FIXED: was window.innerWidth check — now uses isMobile state
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontFamily: 'inherit',
                                    flex: isMobile ? 1 : undefined, // FIXED
                ...
                <div
                    style={{
                        display: 'grid',
                        // FIXED: was window.innerWidth < 1024 — now uses isNarrow state
                ...
                >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        ...
                        <div className="glass-card" style={{ ...glassCard, padding: 18 }}>
                            <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 18 }}>
                                🏆 {locale === 'ru' ? 'Достижения' : 'Achievements'}
                            </div>
                            {/* FIXED: was window.innerWidth check */}
                            <div
                                style={{
                                    maxHeight: isMobile ? 'unset' : 620,
                                    overflowY: 'auto',
                                    paddingRight: 4,
                                }}
                            >
