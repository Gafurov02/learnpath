...
export default function ImportPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function parseQsze(file: File) {
        // qsze = zlib compressed binary format
        const buf = await file.arrayBuffer();
        ...
        function readInt(pos: number) { return dv.getUint32(pos, true); }
        function readStr(pos: number): [string, number] {
            const len = readInt(pos); pos += 4;
            const str = dec.decode(decompressed.slice(pos, pos + len));
            return [str, pos + len];
        }

        // Skip header: version(4), title_len+title, padding(4), unknown(4), ts(8), qcount(4), padding(8)
        let p = 4;
